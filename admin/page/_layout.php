...
function admin_header(string $title, string $active): void { global $user; ?>
<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body class="bg-[#f2f2f6] text-slate-900 antialiased">
    <header
      class="fixed inset-x-5 top-0 z-30 flex h-16 items-center justify-between bg-[#f2f2f6]/95 backdrop-blur"
    >
      <a
        href="overviews.php"
        class="text-2xl font-semibold tracking-tight uppercase"
        >solis skin</a
      >
      <div class="flex items-center gap-4">
        ...
        >
          <span
            ...
            ><?= e(strtoupper(substr($user['username'], 0, 1))) ?></span
          >
    ...
    >
      ...
      <a class="group absolute w-[calc(256px-20px)] h-10 flex justify-center items-center bottom-2.5 bg-white text-black rounded-[25px] transition duration-200 hover:bg-black hover:text-white" href="../../auth/logout.php">
    ...
    <main class="min-h-screen pt-16 md:ml-64">
      <div
        class="min-h-[calc(100vh-4rem)] rounded-tl-[30px] bg-white p-4 md:p-6"
      >
        <?php } function admin_nav_group(string $label, array $items, string
        $active): void { ?>
        <section>
          ...
          <div class="rounded-[25px] bg-white p-[5px] flex flex-col gap-1">
            ...
            <a
              href="<?= e($href) ?>"
              class="flex h-10 items-center gap-3 rounded-[30px] px-3 text-sm transition <?= $isActive ? 'bg-black text-white' : 'text-slate-500 hover:bg-black hover:text-white' ?>"
        ...
        <?php } function admin_footer(): void { ?>
      </span>
    </main>
  </body>
</html>
<?php } function page_title(string $eyebrow, string $title, string $description,
?array $action = null): void { ?>
<div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
  ...
  <?php if ($action): ?><a
    href="<?= e($action[0]) ?>"
    class="rounded-full bg-black px-5 py-3 text-sm text-white transition hover:bg-slate-700"
    ><i class="bi bi-<?= e($action[2]) ?> mr-1"></i
    ><?= e($action[1]) ?></a
  ><?php endif; ?>
</div>
<?php }
