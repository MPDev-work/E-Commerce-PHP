...
'admin'");
$completedOrders = scalar($conn, "SELECT COUNT(*) total FROM orders
WHERE status = 'completed'");
$cancelledOrders = scalar($conn, "SELECT COUNT(*)
total FROM orders WHERE status = 'cancelled'");
$revenue = scalar($conn, "SELECT
...
p_image FROM products ORDER BY p_id DESC LIMIT 6');
admin_header(
  'Overview',
  'overviews.php'
);
page_title(
  ...
  'Here is a live summary of your store today.',
  ['addProduct.php', 'Add product', 'plus-circle']
); ?>
<section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
  <?php foreach (
    [['box-seam', 'Products', $productCount, 'bg-blue-500/15
  ...
  text-indigo-500'], ['currency-dollar', 'Revenue', '$' .
    ...
    <article class="rounded-[24px] bg-[#f2f2f6] p-4">
      <div class="flex items-center gap-3">
        <span class="flex h-12 w-12 items-center justify-center rounded-full text-xl <?= $colourClass ?>"><i
...
<section class="mt-6">
  <div class="mb-3 flex items-end justify-between">
    ...
    <a class="text-sm text-emerald-600 hover:underline" href="products.php">View all</a>
  </div>
            ...
            <tr class="border-b border-slate-100 last:border-0 hover:bg-[#f2f2f6]">
              <td class="flex items-center gap-3 px-5 py-3 font-medium">
                <?php if ($product['p_image']): ?><img src="../../images/<?= e($product['p_image']) ?>"
                    class="h-11 w-11 rounded-xl object-cover" /><?php endif; ?><?= e($product['p_title']) ?>
              </td>
              <td class="px-5 py-3">
